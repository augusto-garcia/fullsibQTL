 ...
 * onemap_addlog
 *
 * Calculate onemap_addlog(a,b) = log[exp(a) + exp(b)]
 *
 * This makes use of the function log1p(x) = log(1+x) provided
 * in R's math library.   
 *
 **********************************************************************/
double onemap_addlog(double a, double b)
{
 ...
 *
 **********************************************************************/
void onemap_reorg_geno(int n_ind, int n_pos, int *geno, int ***Geno)
{
  int i;
 ...
 *
 **********************************************************************/
void onemap_reorg_genoprob(int n_ind, int n_pos, int n_gen, 
 ...
 *
 **********************************************************************/
void onemap_allocate_alpha(int n_pos, int n_gen, double ***alpha)
{
  int i;
 ...
 *
 **********************************************************************/
void onemap_allocate_double(int n, double **vector)
{
 ...
 *
 **********************************************************************/
void onemap_allocate_int(int n, int **vector)
{
 ...
 *
 **********************************************************************/
void onemap_allocate_dmatrix(int n_row, int n_col, double ***matrix)
{
  int i;
 ...
 *
 **********************************************************************/
void onemap_allocate_imatrix(int n_row, int n_col, int ***matrix)
{
  int i;
...
double onemap_init_outbred(int true_gen)
{
  return(LN_025);
}

double onemap_emit_outbred(int obs_gen, int true_gen, double error_prob, int mark_type)
{  
...
double onemap_step_outbred(int gen1, int gen2, int phase, double rf)
{
  if(phase==1){/*CC*/
    switch(gen1) {
    case 1:
...
double onemap_nrec_outbred(int gen1, int gen2, int phase){
  if(phase==1){
    switch(gen1) {
    case 1:
      switch(gen2) {
...
void onemap_est_map(int n_ind, int n_mar, int *type, int *phase, int n_gen, int *geno, double *rf, 
	     ...
	     int verbose)
{
  ...
  onemap_reorg_geno(n_ind, n_mar, geno, &Geno);
  onemap_allocate_alpha(n_mar, n_gen, &alpha);
  onemap_allocate_alpha(n_mar, n_gen, &beta);
  onemap_allocate_dmatrix(n_gen, n_gen, &gamma);
  onemap_allocate_double(n_mar-1, &cur_rf);
    ...
    */
	...
	for(v=0; v<n_gen; v++) {
	  ...
	  for(v2=1; v2<n_gen; v2++) {
	    alpha[v][j] = onemap_addlog(alpha[v][j], alpha[v2][j-1] + 
				 stepf(v2+1,v+1,phase[j-1],cur_rf[j-1]));
	    beta[v][j2] = onemap_addlog(beta[v][j2], beta[v2][j2+1] + 
	...
	for(v=0, s=0.0; v<n_gen; v++) {
	  for(v2=0; v2<n_gen; v2++) {
	    ...
	    else s = onemap_addlog(s, gamma[v][v2]);
	  }
	}
  ...
  *loglik = 0.0;
  for(i=0; i<n_ind; i++) { /* i = individual */
    ...
    for(v=1; v<n_gen; v++) 
      curloglik = onemap_addlog(curloglik, alpha[v][n_mar-1]);
    *loglik += curloglik;
  }
...
void onemap_calc_genoprob(int n_ind, int n_pos, int *type, int *phase, int n_gen, int *geno, 
		   double *rf,  
		   ...
		   double stepf(int, int, int, double)) 
{
  ...
  onemap_reorg_geno(n_ind, n_pos, geno, &Geno);
  onemap_reorg_genoprob(n_ind, n_pos, n_gen, genoprob, &Genoprob);
  onemap_allocate_alpha(n_pos, n_gen, &alpha);
  onemap_allocate_alpha(n_pos, n_gen, &beta);
  ...
  for(i=0; i<n_ind; i++) { /* i = individual */
	...
	s = onemap_addlog(s, Genoprob[v][j][i]);
      }
      for(v=0; v<n_gen; v++) 
...
void onemap_est_map_outbred(int *n_ind, int *n_mar, int *type, int *phase, int *geno, double *rf,
		     double *error_prob, double *loglik, int *maxit, 
		     double *tol, int *verbose)
{
  onemap_est_map(*n_ind, *n_mar, type, phase, 4, geno, rf, *error_prob, 
	  onemap_init_outbred, onemap_emit_outbred, onemap_step_outbred, onemap_nrec_outbred,
	  loglik, *maxit, *tol, *verbose);
}


void onemap_calc_genoprob_outbred(int *n_ind, int *n_mar, int *type,  int *phase, int *geno, 
			   double *rf, double *error_prob, double *genoprob) 

{
  onemap_calc_genoprob(*n_ind, *n_mar, type, phase, 4, geno, rf, *error_prob, genoprob,
		onemap_init_outbred, onemap_emit_outbred, onemap_step_outbred);
}     
