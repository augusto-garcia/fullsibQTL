...
# draw.phase that used the result of this function...                 #
# all the MLE and tests are done in C code                            #
#######################################################################

cim.char <- function(fullsib, pheno.col=1, ws = 10, lg, pos,
                     maxit=1000,tol=1e-10, icim=FALSE, verbose=FALSE)
{
  ...
  cim.models <- model.select.lg(fullsib, ws = ws, LG=lg)
  ...
  structure(z, class = c("fullsib.char", "matrix"))
