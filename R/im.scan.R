...
# im.scan is based on scanone function from qtl package               #
#                                                                     #
...
im.scan <- function (fullsib,lg, pheno.col=1, addcovar=NULL, LOD=TRUE,
                     ...
                     verbose, verbose.scan)
{
  ...
  if (!missing(n.perm) && n.perm > 0) {
    ...
    for (i in 1:n.perm) {
      ...
      temp <- im.scan(fullsib, lg=lg, pheno.col=pheno.col, addcovar=addcovar, maxit=maxit, tol=tol, LOD=LOD, verbose=verbose,
  ...
  class(scan.results) <- c("fullsib.scan", "matrix")
  scan.results
}
