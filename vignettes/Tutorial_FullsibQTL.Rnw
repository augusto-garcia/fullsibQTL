...
Gazaffi, R, Amadeu, RR, Bachega, JRBF, Garcia, AAF (in prep). A application note of this package. \emph{Journal}. Vol:Pages.
...
You can also install the development version hosted at github:
<<eval=FALSE, echo=FALSE>>= 
library(devtools)
...
\textsl{FullsibQTL} package is composed by set of functions present in the Table~\ref{tab:fun}.
...
\textsl{fullsibQTL} uses the same data format of \textsl{onemap}. For more information about it, please see how to create the data file at \textsl{onemap} tutorial. Following, there is an example of this data format with 10 individuals, 5 markers, and 2 phenotypic traits.
...
Within this file, you can now read the data:
<<eval=FALSE>>=
fs.data <- read.outcross.pheno("C:/workingdirectory","filename.txt")
@ 

With your set directory is the same of you file:
<<eval=FALSE>>=
